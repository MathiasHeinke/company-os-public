...
Local EVE registry and hygiene helpers:

```text
scripts/operator-shell/eve-session-registry-core.mjs
scripts/operator-shell/eve-session-registry.mjs
```

`start_eve.mjs check` writes the installed workspace's local/private registry
under the generated EVE context root. The registry stores route receipts,
generation, hygiene state and empty runtime session-id slots. It must not store
secrets, cookies, raw customer data or hidden live session content.
...
The initial Command EVE registry file is:

```text
${COMPANY_OS_PRIVATE_ROOT}/aion-companyos-context/EVE_WORKSTREAM_SESSIONS.json
```

It is generated by `/start_eve`, not committed to Company.OS.
...
The local registry evaluates each open workstream before reuse:

- `pass`: session may remain open for proposal-only work.
- `needs_close_stale`: session exceeded its stale threshold and must rotate or
  close before real resume.
- `blocked_polluted`: context pollution was detected; `/start_eve` blocks until
  the workstream is closed/reset.

`/start_eve` currently rotates stale generations and blocks polluted state. It
does not resume live CLI sessions yet.
...
2. `1.0`: live workstream session registry plus real close-session hygiene.
...
node --test scripts/operator-shell/eve-session-registry-core.test.mjs
